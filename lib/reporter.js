...
class CucumberJsJsonReporter extends WDIOReporter {
    ...
    getStepDataObject(stepData) {
        const dialect = dialects()[this.options.language];
        const keywordList = []
            .concat(dialect.given, dialect.when, dialect.then, dialect.and, ['After', 'Before'])
            .map((keyword) => keyword.trimEnd())
            .filter((keyword) => keyword !== '*');
        const keyword = stepData.keyword
            || (stepData.title.match(new RegExp(`^(${ keywordList.join('|') })\\s`)) || []).pop()
            || '';
