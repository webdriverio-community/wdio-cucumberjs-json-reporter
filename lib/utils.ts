...
export default class Utils {
    ...
    public getFailedMessage(testObject: TestStats | HookStats): ErrorMessage {
        if (testObject.state === FAILED && testObject.error && testObject.error.stack) {
            return {
    ...
    public containsSteps(steps: Step[], language: string): boolean {
        ...
        return steps.some((step) => step.keyword && stepKeywords.includes(step.keyword));
    }
    ...
    public keywordStartsWith(title: string, language: string): string | undefined {
        const stepKeywords = ([] as string[]).concat(this.getStepKeywords(language), ['After', 'Before']);
